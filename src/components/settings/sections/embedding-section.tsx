...
import { testEmbeddingConnection } from "@/lib/connection-tests"
...
export function EmbeddingSection({ draft, setDraft }: Props) {
  ...
  }, [draft.embeddingEnabled, draft.embeddingEndpoint, draft.embeddingApiKey, draft.embeddingModel, t])
            ...
            <div className="space-y-1.5">
              ...
              >
                {connection.kind === "testing"
                  ...
                  : t("settings.testEmbeddingConnection")}
              </Button>
              {connection.kind !== "idle" && connection.kind !== "testing" && (
                <p
                  role={connection.kind === "error" ? "alert" : "status"}
                  className={`text-xs ${
                    connection.kind === "success" ? "text-emerald-600" : "text-destructive"
                  }`}
                >
                  {connection.message}
                </p>
