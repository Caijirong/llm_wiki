...
import { ChevronDown, ChevronRight, AlertCircle, CheckCircle2, Loader2, RefreshCw, XCircle } from "lucide-react"
import { useTranslation } from "react-i18next"
import { invoke } from "@tauri-apps/api/core"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import { useWikiStore, type LlmConfig, type ProviderOverride } from "@/stores/wiki-store"
...
function PresetRow({
  ...
  async function handleTestConnection() {
    setTestState({ status: "testing", message: t("settings.testingConnection") })
    try {
      const result = await testLlmConnection(resolveConfig(preset, ov, fallbackConfig))
      setTestState({
        status: "success",
      ...
      {isExpanded && (
        <div className="space-y-4 border-t bg-background/50 px-4 py-3">
          ...
          {preset.provider !== "claude-code" && (
            <ConnectionTestButton state={testState} onClick={handleTestConnection} />
          )}
        </div>
      )}
    </div>
  ...
  return (
    <div className="space-y-1.5">
      <button
        ...
        onClick={onClick}
        disabled={testing}
        className="inline-flex items-center gap-2 rounded-md border border-border px-2.5 py-1.5 text-xs text-muted-foreground hover:bg-accent hover:text-accent-foreground disabled:opacity-60"
      >
        <Icon className={`h-3.5 w-3.5 ${testing ? "animate-spin" : ""}`} />
        {testing ? t("settings.testingConnection") : t("settings.testLlmConnection")}
      </button>
      {state.status !== "idle" && (
        <p
