...
const EXPECTED_LOCAL_LLM_ORIGIN = "http://localhost"

describe("streamChat against a fake Ollama server (real TCP)", () => {
  it(
    "sends Origin = http://localhost so Ollama's default CORS allowlist passes",
    async () => {
      ...
      try {
        server.setRejectMode({
          kind: "accept-same-origin",
          selfOrigin: EXPECTED_LOCAL_LLM_ORIGIN,
        })
        ...
        expect(reqs[0].origin).toBe(EXPECTED_LOCAL_LLM_ORIGIN)
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        expect(state.done).toBe(true)
        expect(server.receivedRequests()[0].origin).toBe(EXPECTED_LOCAL_LLM_ORIGIN)
      } finally {
        await server.close()
      }
    },
