...
function buildOpenAiBody(
  ...
  const { stream = true, ...rest } = overrides ?? {}
  const translated = messages.map((m) => ({
    role: m.role,
    content: toOpenAiContent(m.content),
  }))
  return { messages: translated, stream, ...rest }
}
...
export function getProviderTestRequest(config: LlmConfig): ProviderTestRequest {
  if (config.provider === "claude-code") {
    throw new Error("Claude Code CLI is checked from the Settings LLM row")
  }
  ...
  return {
    ...
    body: providerConfig.buildBody(TEST_MESSAGES, { max_tokens: 1, stream: false }),
  }
}
