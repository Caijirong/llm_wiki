...
function normalizeTask(raw: unknown, projectId: string): IngestTask | null {
  ...
  return {
    ...
    projectId: typeof task.projectId === "string" && task.projectId.length > 0 ? task.projectId : projectId,
...
export async function cancelAllTasks(): Promise<number> {
  ...
  queue = queue.filter((t) => t.status === "failed" || t.status === "done")
 ...
 */
export async function syncQueueFromDisk(
  projectId: string,
  projectPath: string,
): Promise<void> {
  const pp = normalizePath(projectPath)
  ...
  const mine = saved.filter((task) => task.projectId === projectId)
...
async function processNext(projectId: string): Promise<void> {
  ...
  try {
    ...
    // Success: keep as done for persisted MCP-visible history.
    currentAbortController = null
    lastWrittenFiles = []
    next.status = "done"
    next.error = null
